...
#
#frap script changes to calculate FPKM for rna-seq data analysis. 
#Fragments per kilobase of transcript per million fragments mapped 
#FPKM=((# hits to i)/(# reads in dataset/1,000,000))/(length of i)
#FPKM=(# hits to i * 1000 * 1,000,000)/(length of i) * (# reads in dataset)
...
foreach(@glob_ARGV) {
	my $f_id=$_;
	foreach(@order) {
		$norm{$f_id}->{$_}=($list{$f_id}->{$_}*1000*1000000)/($Tj{$samples{$f_id}}*($length{$_}));
#		print STDERR "$_\t$f_id\t$list{$f_id}->{$_}\t$Tj{$samples{$f_id}}\t$length{$_}\n";
	}
}
