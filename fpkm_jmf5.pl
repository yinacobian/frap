...
#fix to calculate fpkm from a sam file 11/15/2018
...
#foreach(@IDS) {
#$run="smalt map -n 40 -f sam -y 0.99 -o $ARGV[2]/$_"."_vs_$db_id.sam $db_id_path $ARGV[1]/$_.fasta";
#print "$run\n";
#system $run;
#}
...
##5. Normalization as FPKM

$run="perl fpkm_from_frap_normalization.pl -t $ARGV[2]/tj.txt -n -l $ARGV[4] -f $ARGV[0] $ARGV[2]/*.tab > $ARGV[2]/all_fpkm_normalized.txt";
print "$run\n";
system $run;
